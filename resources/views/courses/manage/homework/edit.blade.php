...
<div class="container mundb-standard-container">
    ...
    <div class="mb-5">
        ...
        <card class="mb-3">
            <h5><i class="MDI pen"></i> 设置作业</h5>
            <div>
                <div class="form-group" style="padding-top: 2.75rem;">
                    <label for="homework_status" class="bmd-label-floating">作业功能</label>

                    <div class="radio">
                        <label>
                            <input type="radio" name="homework_status" id="homework_status_1" value="1" <{ if $syllabus_info[ 'homework']!=="0" }>checked
                            <{/if}> required>开启
                        </label>
                    </div>
                    <div class="radio">
                        <label>
                            <input type="radio" name="homework_status" id="homework_status_0" value="0" <{ if $syllabus_info[ 'homework']==="0" }>checked
                            <{/if}> required>关闭
                        </label>
                    </div>
                </div>
            ...
            <div class="text-right">
                <button type="submit" class="btn btn-outline-primary" onclick="updateHomeworkSettings()">更新</button>
            </div>
        </card>
        <card class="mb-3">
            <h5><i class="MDI pen"></i> 设置作业内容</h5>
                ...
                <div class="row">
                    <div class="col-lg-4 col-12">
                        <div class="form-group">
                            <label for="homework_type_<{$h['hid']}>" class="bmd-label-floating">形式</label>
                            <select class="form-control" id="homework_type_<{$h['hid']}>" name="homework_type_<{$h['hid']}>" required>
                    ...
                    <div class="col-lg-4 col-12">
                        <div class="form-group">
                            <label for="homework_ddl_<{$h['hid']}>" class="bmd-label-floating">截止日期</label>
                            <input type="text" class="form-control" name="homework_ddl_<{$h['hid']}>" value="<{$h['due_submit']}>" id="homework_ddl_<{$h['hid']}>" required>
                        </div>
                    </div>
                    <div class="col-lg-4 col-12">
                        <div class="form-group">
                            <label for="homework_support_lang_<{$h['hid']}>" class="bmd-label-floating">格式</label>
                            <input type="text" class="form-control" name="homework_support_lang_<{$h['hid']}>" value="<{$h['support_lang']}>" id="homework_support_lang_<{$h['hid']}>" required>
                        </div>
                    </div>
                </div>
                <tips>内容</tips>
                <div id="vscode_container_<{$h['hid']}>">
                    <div id="vscode_<{$h['hid']}>" style="width:100%;height:30rem;border:1px solid grey"></div>
                </div>
            </div>
            <div class="text-right">
                <button type="submit" class="btn btn-outline-info" data-hid="<{$h['hid']}>" onclick="updateHomework(this)">修改作业内容</button>
            </div>
            <hr class="atsast-line atsast-divider">
            <{/foreach}>

            <div class="text-center">
                <button type="button" class="btn btn-outline-warning atsast-important-button" onclick="location.href='<{$ATSAST_DOMAIN}>/course/<{$cid}>/add_homework/<{$syid}>'"><action class="d-block d-lg-inline-block text-warning"><i class="MDI plus"></i> 新增作业项</action></button>
            </div>
        </card>
    </div>
</div>
...
<script>
    ...
    window.addEventListener("load",function() {
        ...
        <{ foreach $homework as $h }>
        $("#homework_type_<{$h['hid']}>").val("<{$h['type']}>");
        <{/foreach}>
    },false);
            ...
            if(jsCnt==1){
                ...
                require(["vs/editor/editor.main"], function () {
                    ...
                    editor[<{$h['hid']}>] = monaco.editor.create(document.getElementById('vscode_<{$h["hid"]}>'), {
                        value: "<{ if isset($h['homework_content_slashed'])}><{$h['homework_content_slashed'] nofilter}><{/if}>",
                        language: "markdown"
                    });
                    $("#vscode_container_<{$h['hid']}>").css("opacity",1);
                    <{/foreach}>
                });
            }
        }
    ...
    function updateHomework(ele){
        var hid=$(ele).attr("data-hid");
        editor[hid].updateOptions({ readOnly: true });
        $.post("<{$ATSAST_DOMAIN}>/ajax/updateHomework",{
            cid:<{$cid}>,
            syid:<{$syid}>,
            hid:hid,
            type:$("#homework_type_"+hid).val(),
            support_lang:$("#homework_support_lang_"+hid).val(),
            due_submit:$("#homework_ddl_"+hid).val(),
            homework_content:editor[hid].getValue(),
        },function(result){
            result=JSON.parse(result);
            $.snackbar({content: result.desc,style:"toast text-center atsast-toast"});
            editor[hid].updateOptions({ readOnly: false });
        });
    }

    function updateHomeworkSettings(){

        $.post("<{$ATSAST_DOMAIN}>/ajax/updateHomeworkSettings",{
            cid:<{$cid}>,
            syid:<{$syid}>,
            homework_status:parseInt($('input[name="homework_status"]:checked').val())
        },function(result){
            result=JSON.parse(result);
            $.snackbar({content: result.desc,style:"toast text-center atsast-toast"});
        });
