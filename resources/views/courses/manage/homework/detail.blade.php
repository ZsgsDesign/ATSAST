...
<div class="container mundb-standard-container">
    ...
    <section>
        <user-card class="mb-3">
            <div>
                <img src="<{$user_details['avatar']}>" style="border-radius:200px;" onerror="this.src=unknown_svg;">
            </div>
            <div>
                <h5 class="mundb-text-truncate-1 text-info"><{$user_details['real_name']}> <{$user_details['SID']}></h5>
                <p class=""><{$user_details['email']}></p>
            </div>
        </user-card>
    </section>
    ...
    <section class="mb-5">
        ...
        <div class="mb-5" id="markdwon_submit_section_<{$h['hid']}>">
            <div id="markdwon_editor_<{$h['hid']}>" style="width:100%;height:30rem;border:1px solid grey"></div>
            <div class="atsast-action"><p>截止日期 <{$h['due_submit']}></p></div>
            <div class="atsast-action">
                <small id="info_submit_<{$h['hid']}>" class="text-secondary"><{ if @$homework_submit[$h['hid']] }>已于 <{ $homework_submit[$h['hid']]['submit_time'] }> 提交<{/if}></small>
            </div>
            ...
            jsNode.onload = function() {
                ...
                if(jsCnt2==2){
                    ...
                    <{ if $h['type']==2}>
                        ...
                        $("#markdwon_editor_<{$h['hid']}>").html(marked("<{ if isset($homework_submit[$h['hid']]['submit_content_slashed'])}><{$homework_submit[$h['hid']]['submit_content_slashed'] nofilter}><{/if}>",{
                            sanitize:true,
                            sanitizer:DOMPurify.sanitize,
                            highlight: function (code) {
                                return hljs.highlightAuto(code).value;
